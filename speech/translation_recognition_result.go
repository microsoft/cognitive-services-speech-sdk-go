...
func NewTranslationRecognitionResultFromHandle(handle uintptr) (*TranslationRecognitionResult, error) {
	result := new(TranslationRecognitionResult)
	result.translations = make(map[string]string)

	// Get base recognition result
	baseResult, err := NewRecognitionResultFromHandle(handle)
	if err != nil {
		return nil, err
	}
	result.RecognitionResult = *baseResult

	// Get translation count
	var count C.size_t
	ret := uintptr(C.translation_text_result_get_translation_count(C.SPXRESULTHANDLE(handle), &count))
	...
	for i := C.size_t(0); i < count; i++ {
		...
		ret = uintptr(C.translation_text_result_get_translation(C.SPXRESULTHANDLE(handle), i, nil, nil, &languageSize, &textSize))
		if ret != C.SPX_NOERROR {
			return nil, common.NewCarbonError(ret)
		}
		...
		ret = uintptr(C.translation_text_result_get_translation(C.SPXRESULTHANDLE(handle), i,
...
func NewTranslationSynthesisResultFromHandle(handle uintptr) (*TranslationSynthesisResult, error) {
	result := new(TranslationSynthesisResult)

	var reason C.Result_Reason
	ret := uintptr(C.result_get_reason(C.SPXRESULTHANDLE(handle), &reason))
	if ret != C.SPX_NOERROR {
		return nil, common.NewCarbonError(ret)
	}
	...
	var size C.size_t
	ret = uintptr(C.translation_synthesis_result_get_audio_data(C.SPXRESULTHANDLE(handle), nil, &size))
	if ret == uintptr(C.SPXERR_BUFFER_TOO_SMALL) {
		result.audioData = make([]byte, size)
		ret = uintptr(C.translation_synthesis_result_get_audio_data(C.SPXRESULTHANDLE(handle),
...
func NewTranslationRecognitionEventArgsFromHandle(handle uintptr) (*TranslationRecognitionEventArgs, error) {
	var resultHandle C.SPXRESULTHANDLE
	ret := uintptr(C.recognizer_recognition_event_get_result(C.SPXEVENTHANDLE(handle), &resultHandle))
	if ret != C.SPX_NOERROR {
		return nil, common.NewCarbonError(ret)
	}

	result, err := NewTranslationRecognitionResultFromHandle(uintptr(resultHandle))
	if err != nil {
		return nil, err
	}

	return &TranslationRecognitionEventArgs{Result: result}, nil
}
...
func NewTranslationRecognitionCanceledEventArgsFromHandle(handle uintptr) (*TranslationRecognitionCanceledEventArgs, error) {
	var reason C.CancellationReason
	var errorCode C.CancellationErrorCode
	var errorDetails *C.char

	ret := uintptr(C.recognizer_recognition_event_get_cancellation_details(C.SPXEVENTHANDLE(handle), &reason, &errorCode, &errorDetails))
	if ret != C.SPX_NOERROR {
		return nil, common.NewCarbonError(ret)
	}
	defer C.free(unsafe.Pointer(errorDetails))

	return &TranslationRecognitionCanceledEventArgs{
		Reason:       common.CancellationReason(reason),
		ErrorCode:    common.CancellationErrorCode(errorCode),
		ErrorDetails: C.GoString(errorDetails),
	}, nil
}
...
func NewTranslationSynthesisEventArgsFromHandle(handle uintptr) (*TranslationSynthesisEventArgs, error) {
	var resultHandle C.SPXRESULTHANDLE
	ret := uintptr(C.recognizer_recognition_event_get_result(C.SPXEVENTHANDLE(handle), &resultHandle))
	if ret != C.SPX_NOERROR {
		return nil, common.NewCarbonError(ret)
	...
	result, err := NewTranslationSynthesisResultFromHandle(uintptr(resultHandle))
	if err != nil {
		return nil, err
	}
