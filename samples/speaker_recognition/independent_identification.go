...
func IndependentIdentification(subscription string, region string, file string) {
	...
	enrollAudioConfig, err := audio.NewAudioConfigFromWavFileInput(file)
	if err != nil {
		fmt.Println("Got an error: ", err)
		return
	}
	defer enrollAudioConfig.Close()
	speakerRecognizer, err := speaker.NewSpeakerRecognizerFromConfig(config, enrollAudioConfig)
	if err != nil {
		fmt.Println("Got an error: ", err)
		return nil
	}
